...
namespace en
{
	ComputeShader::ComputeShader(const CreateInfo& createInfo) : m_SourcePath(createInfo.sourcePath)
	{
		UseContext();
		...
		if (vkCreateComputePipelines(ctx.m_LogicalDevice, VK_NULL_HANDLE, 1U, &pipelineCreateInfo, nullptr, &m_Pipeline) != VK_SUCCESS)
			throw std::runtime_error("ComputeShader::ComputeShader() - Failed to create compute pipeline!");
